	...
	if (config.contains("UserIOConfiguration")) {
		for (const auto& user_io_item_json : config["UserIOConfiguration"]) {
			if (!user_io_item_json.contains("Type")) {
				...
				else if (type == "DDSAD9959") {
					UserIOItemList.push_back(std::make_shared<CUserIOItemAD9959>(user_io_item_json));
				}
				else {
					CString message;
...
bool CUserIOItemNewMenu::AssembleIOList() {
	IORegisterList->NewMenu((Text + " Control Menu").c_str());
	return true;
}
...
bool CUserIOItemStaticText::AssembleIOList() {
	IORegisterList->AddStatic(Text.c_str(), Help.c_str(),  Color);
	return true;
}
...
bool CUserIOItemAnalogOut::AssembleIOList() {
	...
	IORegisterList->RegisterAnalogOutputStdFunction(
...
bool CUserIOItemDigitalOut::AssembleIOList() {
	...
	IORegisterList->RegisterDigitalOutputStdFunction(
...
//Now define everything needed for AD9959
CUserIOItemAD9959::CUserIOItemAD9959(const json& user_io_item_json) {
	Name = GET_VALUE("Name", "DDSAD9959NoName");
	Description = GET_VALUE("Description", "");
	Help = GET_VALUE("Help", "");
	Sequencer = GET_VALUE("Sequencer", 0);
	Address = GET_VALUE("Address", 0);
	...
	InitFrequencyCh2 = GET_VALUE("InitFrequencyCh2", 0.0);
	InitPowerCh2 = GET_VALUE("InitPowerCh2", 100.0);
	InitPhaseCh2 = GET_VALUE("InitPhaseCh2", 0.0);

	InitFrequencyCh3 = GET_VALUE("InitFrequencyCh3", 0.0);
	InitPowerCh3 = GET_VALUE("InitPowerCh3", 100.0);
	InitPhaseCh3 = GET_VALUE("InitPhaseCh3", 0.0);

	Hardreset = GET_VALUE("Hardreset", false);
	Softreset = GET_VALUE("Softreset", true);
	BlockManualControl = GET_VALUE("BlockManualControl", false);

	FindMyChannel("AD9959");

	AD9958 = AD9959[MyChannelNr]->AD9958;
...
bool CUserIOItemAD9959::AssembleIOList() {
	CString ChannelNumberString;
	ChannelNumberString.Format(" (AD9959 software channel %u)", MyChannelNr);
	if (FrequencyControl == "FrequencyInMHz") {
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetFrequencyCh0, this, std::placeholders::_1),
		...
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetFrequencyTuningWordCh0, this, std::placeholders::_1),
	...
	if (PowerControl == "PowerIndB") {
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetAttenuationCh0, this, std::placeholders::_1),
			("SetAttenuationCh0" + Name).c_str(),
			(Description + " Attenuation Ch0").c_str(),
			"-32..0dB",
			Help.c_str()+ ChannelNumberString, DDSAD9959AttenuationMax, 0, BlockManualControl, Color);
	}
	else {
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetIntensityCh0, this, std::placeholders::_1),
	...
	IORegisterList->RegisterAnalogOutputStdFunction(
	std::bind(&CUserIOItemAD9959::SetPhaseCh0, this, std::placeholders::_1),
	...
	if (FrequencyControl == "FrequencyInMHz") {
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetFrequencyCh1, this, std::placeholders::_1),
		...
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetFrequencyTuningWordCh1, this, std::placeholders::_1),
	...
	if (PowerControl == "PowerIndB") {
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetAttenuationCh1, this, std::placeholders::_1),
		...
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetIntensityCh1, this, std::placeholders::_1),
	...
	IORegisterList->RegisterAnalogOutputStdFunction(
		std::bind(&CUserIOItemAD9959::SetPhaseCh1, this, std::placeholders::_1),
	...
	if (!AD9958) {
		if (FrequencyControl == "FrequencyInMHz") {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetFrequencyCh2, this, std::placeholders::_1),
				("SetFrequencyCh2" + Name).c_str(),
				(Description + " Frequency Ch2").c_str(),
				"MHz",
				Help.c_str() + ChannelNumberString, MinFrequency, MaxFrequency, BlockManualControl, Color);
		}
		else {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetFrequencyTuningWordCh2, this, std::placeholders::_1),
				("SetFrequencyTuningWordCh2" + Name).c_str(),
				(Description + " Frequency Tuning Word Ch2").c_str(),
				"",
				Help.c_str() + ChannelNumberString, MinFrequency, MaxFrequency, BlockManualControl, Color);
		}
		...
		if (PowerControl == "PowerIndB") {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetAttenuationCh2, this, std::placeholders::_1),
				("SetAttenuationCh2" + Name).c_str(),
				(Description + " Attenuation Ch2").c_str(),
				"-32..0dB",
				Help.c_str() + ChannelNumberString, DDSAD9959AttenuationMax, 0, BlockManualControl, Color);
		}
		else {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetIntensityCh2, this, std::placeholders::_1),
				("SetIntensityCh2" + Name).c_str(),
				(Description + " Intensity Ch2").c_str(),
				"%",
				Help.c_str(), 0, 100, BlockManualControl, Color);
		}
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetPhaseCh2, this, std::placeholders::_1),
			("SetPhaseCh2" + Name).c_str(),
			(Description + " Phase Ch2").c_str(),
			"",
			Help.c_str() + ChannelNumberString, 0, 360, BlockManualControl, Color);

		if (FrequencyControl == "FrequencyInMHz") {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetFrequencyCh3, this, std::placeholders::_1),
				("SetFrequencyCh3" + Name).c_str(),
				(Description + " Frequency Ch3").c_str(),
				"MHz",
				Help.c_str() + ChannelNumberString, MinFrequency, MaxFrequency, BlockManualControl, Color);
		}
		else {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetFrequencyTuningWordCh3, this, std::placeholders::_1),
				("SetFrequencyTuningWordCh3" + Name).c_str(),
				(Description + " Frequency Tuning Word Ch3").c_str(),
				"",
				Help.c_str() + ChannelNumberString, MinFrequency, MaxFrequency, BlockManualControl, Color);
		}
		...
		if (PowerControl == "PowerIndB") {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetAttenuationCh3, this, std::placeholders::_1),
				("SetAttenuationCh3" + Name).c_str(),
				(Description + " Attenuation Ch3").c_str(),
				"-32..0dB",
				Help.c_str() + ChannelNumberString, DDSAD9959AttenuationMax, 0, BlockManualControl, Color);
		}
		else {
			IORegisterList->RegisterAnalogOutputStdFunction(
				std::bind(&CUserIOItemAD9959::SetIntensityCh3, this, std::placeholders::_1),
				("SetIntensityCh3" + Name).c_str(),
				(Description + " Intensity Ch3").c_str(),
				"%",
				Help.c_str(), 0, 100, BlockManualControl, Color);
		}
		IORegisterList->RegisterAnalogOutputStdFunction(
			std::bind(&CUserIOItemAD9959::SetPhaseCh3, this, std::placeholders::_1),
			("SetPhaseCh3" + Name).c_str(),
			(Description + " Phase Ch3").c_str(),
			"",
			Help.c_str() + ChannelNumberString, 0, 360, BlockManualControl, Color);
	}
...
bool CUserIOItemAD9959::AssembleParamList() {
	if (FrequencyControl == "FrequencyInMHz") {
	...
	if (!AD9958) {
		...
		if (FrequencyControl == "FrequencyInMHz") {
			ParamList->RegisterDouble(&InitFrequencyCh2, ("InitFrequencyCh2" + Name).c_str(), MinFrequency, MaxFrequency, (Description + " Frequency Ch2").c_str(), "MHz");
		}
		else {
			ParamList->RegisterDouble(&InitFrequencyCh2, ("InitFrequencyTuningWordCh2" + Name).c_str(), MinFrequency, MaxFrequency, (Description + " Frequency Tuning Word Ch2").c_str(), "");
		}

		if (PowerControl == "PowerIndB") {
			ParamList->RegisterDouble(&InitPowerCh2, ("InitAttenuationCh2" + Name).c_str(), 0, 10, (Description + " Attenuation Ch2").c_str(), "-32..0dB");
		}
		else {
			ParamList->RegisterDouble(&InitPowerCh2, ("InitIntensityCh2" + Name).c_str(), 0, 100, (Description + " Intensity Ch2").c_str(), "0..100%");
		}
		ParamList->RegisterDouble(&InitPhaseCh2, ("InitPhaseCh2" + Name).c_str(), 0, 100, (Description + " Phase Ch2").c_str(), "");


		if (FrequencyControl == "FrequencyInMHz") {
			ParamList->RegisterDouble(&InitFrequencyCh3, ("InitFrequencyCh3" + Name).c_str(), MinFrequency, MaxFrequency, (Description + " Frequency Ch3").c_str(), "MHz");
		}
		else {
			ParamList->RegisterDouble(&InitFrequencyCh3, ("InitFrequencyTuningWordCh3" + Name).c_str(), MinFrequency, MaxFrequency, (Description + " Frequency Tuning Word Ch3").c_str(), "");
		}

		if (PowerControl == "PowerIndB") {
			ParamList->RegisterDouble(&InitPowerCh3, ("InitAttenuationCh3" + Name).c_str(), 0, 10, (Description + " Attenuation Ch3").c_str(), "-32..0dB");
		}
		else {
			ParamList->RegisterDouble(&InitPowerCh3, ("InitIntensityCh3" + Name).c_str(), 0, 100, (Description + " Intensity Ch3").c_str(), "0..100%");
		}
		ParamList->RegisterDouble(&InitPhaseCh3, ("InitPhaseCh3" + Name).c_str(), 0, 100, (Description + " Phase Ch3").c_str(), "");
...
void CUserIOItemAD9959::Reset(bool OnlyFastOutputs, bool HardResetSystem) {
	// Reset implementation
	if (HardResetSystem) {
		// Perform hard reset
		AD9959[MyChannelNr]->MasterReset();
	}
	...
	if (!AD9958) {
		if (FrequencyControl == "FrequencyInMHz") {
			SetFrequencyCh2(InitFrequencyCh2);
		}
		else {
			SetFrequencyTuningWordCh2(InitFrequencyCh2);
		}
		if (PowerControl == "PowerIndB") {
			SetAttenuationCh2(InitPowerCh2);
		}
		else {
			SetIntensityCh2(InitPowerCh2);
		}
		SetPhaseCh2(InitPhaseCh2);


		if (FrequencyControl == "FrequencyInMHz") {
			SetFrequencyCh3(InitFrequencyCh3);
		}
		else {
			SetFrequencyTuningWordCh3(InitFrequencyCh3);
		}
		if (PowerControl == "PowerIndB") {
			SetAttenuationCh3(InitPowerCh3);
		}
		else {
			SetIntensityCh3(InitPowerCh3);
		}
		SetPhaseCh3(InitPhaseCh3);


	}
}

void CUserIOItemAD9959::SetIntensityCh0(double Intensity) {
	double OutPower = EvaluatePolynomial(Intensity, powerCalibrationTerms);
	if (OutPower > 100.0) OutPower = 100.0;
	if (OutPower < 0.0) OutPower = 0.0;
	SetIntensityCh0DDSAD9959(MyChannelNr, OutPower);
}

void CUserIOItemAD9959::SetAttenuationCh0(double Attenuation) {
	SetAttenuationCh0DDSAD9959(MyChannelNr, Attenuation);
}

void CUserIOItemAD9959::SetFrequencyCh0(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyCh0DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetFrequencyTuningWordCh0(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyTuningWordCh0DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetPhaseCh0(double Phase) {
	SetPhaseCh0DDSAD9959(MyChannelNr, Phase);
}


void CUserIOItemAD9959::SetIntensityCh1(double Intensity) {
	double OutPower = EvaluatePolynomial(Intensity, powerCalibrationTerms);
	if (OutPower > 100.0) OutPower = 100.0;
	if (OutPower < 0.0) OutPower = 0.0;
	SetIntensityCh1DDSAD9959(MyChannelNr, OutPower);
}

void CUserIOItemAD9959::SetAttenuationCh1(double Attenuation) {
	SetAttenuationCh1DDSAD9959(MyChannelNr, Attenuation);
}

void CUserIOItemAD9959::SetFrequencyCh1(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyCh1DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetFrequencyTuningWordCh1(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyTuningWordCh1DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetPhaseCh1(double Phase) {
	SetPhaseCh1DDSAD9959(MyChannelNr, Phase);
}

void CUserIOItemAD9959::SetIntensityCh2(double Intensity) {
	double OutPower = EvaluatePolynomial(Intensity, powerCalibrationTerms);
	if (OutPower > 100.0) OutPower = 100.0;
	if (OutPower < 0.0) OutPower = 0.0;
	SetIntensityCh2DDSAD9959(MyChannelNr, OutPower);
}

void CUserIOItemAD9959::SetAttenuationCh2(double Attenuation) {
	SetAttenuationCh2DDSAD9959(MyChannelNr, Attenuation);
}

void CUserIOItemAD9959::SetFrequencyCh2(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyCh2DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetFrequencyTuningWordCh2(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyTuningWordCh2DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetPhaseCh2(double Phase) {
	SetPhaseCh2DDSAD9959(MyChannelNr, Phase);
}


void CUserIOItemAD9959::SetIntensityCh3(double Intensity) {
	double OutPower = EvaluatePolynomial(Intensity, powerCalibrationTerms);
	if (OutPower > 100.0) OutPower = 100.0;
	if (OutPower < 0.0) OutPower = 0.0;
	SetIntensityCh3DDSAD9959(MyChannelNr, OutPower);
}

void CUserIOItemAD9959::SetAttenuationCh3(double Attenuation) {
	SetAttenuationCh3DDSAD9959(MyChannelNr, Attenuation);
}

void CUserIOItemAD9959::SetFrequencyCh3(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyCh3DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetFrequencyTuningWordCh3(double Frequency) {
	Output->RangeCheck(Frequency, MinFrequency, MaxFrequency, Name.c_str());
	SetFrequencyTuningWordCh3DDSAD9959(MyChannelNr, Frequency);
}

void CUserIOItemAD9959::SetPhaseCh3(double Phase) {
	SetPhaseCh3DDSAD9959(MyChannelNr, Phase);
}

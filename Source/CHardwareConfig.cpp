...
bool CHardwareConfig::LoadFromJSON(const json& config) {
	...
	if (config.contains("DDSAD9959Boards")) {
		for (const auto& device_json : config["DDSAD9959Boards"])
			success &= AddDeviceAD9959FromJSON(device_json);
	}
...
bool CHardwareConfig::AddDeviceAD9959FromJSON(const json& device_json) {
	if (!device_json.contains("Address")) return false;
	unsigned int address = device_json["Address"];
	unsigned int sequencer = GET_VALUE("Sequencer", 0);
	double externalClockFrequency = GET_VALUE("ClockFrequencyinMHz", 300);
	unsigned int frequencyMultiplier = GET_VALUE("FrequencyMultiplier", 1);
	unsigned int AD9958 = GET_VALUE("AD9958", 0);
	return AddDeviceAD9959(sequencer, address, externalClockFrequency, frequencyMultiplier, AD9958==1);
}


bool CHardwareConfig::AddDeviceAD9959(
	unsigned int sequencer,
	unsigned int address,
	double externalClockFrequencyinMHz,
	unsigned int frequencyMultiplier,
	bool AD9958) {
	// Add the AD9959 board to the sequencer
	if (SequencerList[sequencer]) {
		new CDeviceAD9959(
			SequencerList[sequencer],
			address,
...
CDeviceAD9959::CDeviceAD9959(
	...
	MySequencer->MyMultiIO->AddAD9959(0x03 & (MyAddress >> 8), (MyAddress & 0xff),/*Clockspeed [MHz]*/externalClockFrequencyinMHz,/*FrequencyMultiplier*/ frequencyMultiplier, AD9958); //DDSSoft 0, DDS26 T1 //Clock generator DDS  //real addresses are this Bus+BaseAddress*4+0..Bus+BaseAddress*4+3, internal clock after internal multiplication in MHz
	MySoftwareAddress = NrAD9959 - 1;
}

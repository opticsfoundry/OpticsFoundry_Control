...
CAD9959* AD9959[MaxAD9959];
unsigned long AD9959MultiWriteDeviceNr[MaxAD9959];
unsigned long NrAD9959=0;
...
CMultiIO::CMultiIO(unsigned char aDigBoardNrUsed, CEthernetMultiIOControllerOpticsFoundry* aSequencer)
{
	...
	for (int i=0;i<MaxAD9959;i++) {
		AD9959[i]=NULL;
		AD9959MultiWriteDeviceNr[i]=0;
	}
	NrAD9959=0;
...
CAD9959* CMultiIO::AddAD9959(unsigned short aBus, unsigned int aBaseAddress, double externalClockSpeed,  double FrequencyMultiplier, bool AD9958)
{
	if (aBaseAddress >= 64) {
		Error("CMultiIO::AddAD9959 : BaseAddress too high");
		return NULL;
	}
	bool AddressIsUsed = AddressUsed(aBaseAddress + aBus);
	if (AddressIsUsed) {
		CString buf;
		buf.Format("CMultiIO::AddAD9959 : Address used Bus=%u BaseAddr=%u", aBus / 256, aBaseAddress);
		Error(buf);
		return NULL;
	}
	AddressUsage[aBaseAddress + aBus] = 7;
	CAD9959* AD9959h = new CAD9959(aBus, aBaseAddress, externalClockSpeed, FrequencyMultiplier, AD9958, this);
	MultiWriteDeviceList->AddHead(AD9959h);
	// the following simple list is used in CSequence for direct access of AD9959 functions and during MultiIO::PrepareGetBusContents
	AD9959[NrAD9959] = AD9959h;
	AD9959MultiWriteDeviceNr[NrAD9959] = NrMultiWriteDevices;
	// the following simple list is used during MultiIO::PrepareGetBusContents
	MultiWriteDeviceTable[NrMultiWriteDevices] = AD9959h;
	NrAD9959++;
	NrMultiWriteDevices++;
	if (NrAD9959 >= MaxAD9959) {
		ControlMessageBox("CMultiIO::AddAD9959 : increase MaxAD9959 in IOList.h");
	}
	return AD9959h;
}

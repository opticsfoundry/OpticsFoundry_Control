...
void COutput::SetAD9959Value(unsigned int DDSNr, double UnscaledValue, double dValue, int Type) {
	if (DDSNr == NotConnected) return;
	if (DDSNr >= NrAD9959) {
		ControlMessageBox("COutput::SetAD9959Value  : unknown channel");
		return;
	}
	if ((Type < 0) || (Type > 22)) {
		ControlMessageBox("COutput::SetAD9959Value  : unknown type");
		return;
	}
	if (Mode == IDO_ASSEMBLE_SEQUENCE_LIST_MODE) {
		SequenceList.Add(new CSequenceListMultiWriteDeviceValue(/*TypeOfMultiWriteDevice*/2, AD9959MultiWriteDeviceNr[DDSNr], DDSNr, UnscaledValue, dValue, Type));
		return;
	}
	//On programstart OutScaledStoreMode is used to transfer channelnumbers. No output should be made.
	// If (Mode==IDO_ASSEMBLE_SEQUENCE_LIST_MODE) we don't output anything, but use the Output->AnalogOutScaled/DigitalOutScaled commands below to store the command in the sequence list.
	...
	if ((InOutScaledNormalMode()) && (!(Mode==IDO_ASSEMBLE_SEQUENCE_LIST_MODE) )) {
		...
		case 0:AD9959[DDSNr]->SetIntensityCh0(dValue); break;
		case 1:AD9959[DDSNr]->SetAttenuationCh0(dValue); break;
		case 2:AD9959[DDSNr]->SetFrequencyCh0(dValue); break;
		case 3:AD9959[DDSNr]->SetFrequencyTuningWordCh0(dValue); break;
		case 4:AD9959[DDSNr]->SetPhaseOffsetCh0(dValue); break;

		case 5:AD9959[DDSNr]->SetIntensityCh1(dValue); break;
		case 6:AD9959[DDSNr]->SetAttenuationCh1(dValue); break;
		case 7:AD9959[DDSNr]->SetFrequencyCh1(dValue); break;
		case 8: AD9959[DDSNr]->SetFrequencyTuningWordCh1(dValue); break;
		case 9:AD9959[DDSNr]->SetPhaseOffsetCh1(dValue); break;

		case 10:AD9959[DDSNr]->SetIntensityCh2(dValue); break;
		case 11:AD9959[DDSNr]->SetAttenuationCh2(dValue); break;
		case 12:AD9959[DDSNr]->SetFrequencyCh2(dValue); break;
		case 13: AD9959[DDSNr]->SetFrequencyTuningWordCh2(dValue); break;
		case 14:AD9959[DDSNr]->SetPhaseOffsetCh2(dValue); break;

		case 15:AD9959[DDSNr]->SetIntensityCh3(dValue); break;
		case 16:AD9959[DDSNr]->SetAttenuationCh3(dValue); break;
		case 17:AD9959[DDSNr]->SetFrequencyCh3(dValue); break;
		case 18: AD9959[DDSNr]->SetFrequencyTuningWordCh3(dValue); break;
		case 19:AD9959[DDSNr]->SetPhaseOffsetCh3(dValue); break;

		case 20:AD9959[DDSNr]->MasterReset(); break;
		case 21:AD9959[DDSNr]->SyncIO(); break;
		case 22:AD9959[DDSNr]->SetIOUpdateEnabled(dValue >0.5); break;
	...
	if (Type <= 21) Output->AnalogOutScaled(MultiWriteDeviceOffsetStartNr + AD9959MultiWriteDeviceNr[DDSNr] * NrMultiWriteDeviceSubPorts + Type, UnscaledValue, dValue);
	else if (Type <= 22) Output->DigitalOutScaled(MultiWriteDeviceOffsetStartNr + AD9959MultiWriteDeviceNr[DDSNr] * NrMultiWriteDeviceSubPorts + Type, UnscaledValue > 0.5, dValue > 0.5);
	else ControlMessageBox(" COutput::SetAD9959Value : unknown type");
}
